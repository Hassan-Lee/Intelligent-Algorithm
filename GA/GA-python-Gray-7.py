...
class GA:
        ...
        # 线性排名,几何概型
            ...
            for i in range(self.population_size):
                ...
                for j in range(self.population_size):
                    if (1-num) >= (2/3)**(j+1):
                        # 条件成立就是选第j+1名
                        #print(j,(2/3)**(j+1),1-num,self.fitness_value[j])
    ...
    def graph_draw(self):
        ...
        #print(self.x0, function(np.array(self.x0)))
        if self.mode == 1:
...
test = GA(np.array([-20,-20]), np.array([20,20]), 80, np.array([16, 2]), 80, 0.6, 0.1,'linrank', elitism=True)
#print(test.run())
...
def singlethr():
    for i in range(100):
        global convergentrate
        global pointacc
        global valueacc
        test = GA(np.array([-20, -20]), np.array([20, 20]), 180, np.array([20, 2]), 180, 0.6, 0.6, 'linrank', elitism=True)
        a = test.run()
        print(a)
        if np.linalg.norm(a[0],1)/2 <= 1 or a[2] <= 1:
            Lock.acquire()
            if np.linalg.norm(a[0],1)/2 <= 1:
                convergentrate += 1
                pointacc += (-np.log10(abs(a[0][0]))-np.log10(abs(a[0][1])))/2
            if a[2] <= 1:
                if a[2] == 0.0:
                    valueacc += 16
                else:
                    valueacc += -np.log10(a[2])
            Lock.release()
for i in range(10):
    globals()[str(i)] = threading.Thread(target=singlethr())
    globals()[str(i)].setDaemon(False)
    globals()[str(i)].start()
for i in range(10):
    globals()[str(i)].join()
...
pointacc /= 1000
valueacc /= 1000
print(convergentrate,pointacc,valueacc)
